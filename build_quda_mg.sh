...
echo "=== Building amg_pcg_ddag_baseline, amg_materialize_minv (jobs=${JOBS}) ==="
cmake --build . --target amg_pcg_ddag_baseline amg_materialize_minv -j "${JOBS}"

echo "=== Build complete ==="
echo "Binary: ${BUILD_DIR}/tests/amg_pcg_ddag_baseline"
echo "Binary: ${BUILD_DIR}/tests/amg_materialize_minv"
ls -la "${BUILD_DIR}/tests/amg_pcg_ddag_baseline" "${BUILD_DIR}/tests/amg_materialize_minv" 2>/dev/null || true
